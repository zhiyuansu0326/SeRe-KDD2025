...
class Network_u(nn.Module):
    def __init__(self, dim, hidden_size=100):
        ...
        self.fc_phi = nn.Linear(hidden_size, dim)  # 用于特征表示
        self.fc_y = nn.Linear(hidden_size, 1)  # 用于预测 y
...
class SCLUB(LinUCB_IND):
    def __init__(
        ...
        # 新增：初始化神经网络相关组件
        self.input_dim = d  # 输入维度与原始特征维度相同
        self.hidden_size = hidden_size
        self.output_dim = d  # 网络输出维度为dim（用于 phi_x）
    ...
    def store_info(self, i, x, y, t, r, br=1):
        # 使用神经网络提取特征表示和预测 y
        with torch.no_grad():
            x_tensor = torch.tensor(x, dtype=torch.float32).unsqueeze(0)  # 形状为(1, d)
            phi_x, y_pred = self.network(x_tensor)
            phi_x = phi_x.squeeze().numpy()  # 确保 phi_x 为 d 维数组
            # y_pred = y_pred.squeeze().numpy()  # 可选，如果需要使用 y_pred

        # 使用深度表示 phi_x 代替原始特征 x
        ...
        # 存储训练数据以便后续训练神经网络
        self.history.append((i, x, y))  # 存储原始 x 而不是 phi_x
